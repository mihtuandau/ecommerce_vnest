...
const GRID_CLASS =
  "grid grid-cols-[minmax(150px,1.05fr)_minmax(170px,1fr)_minmax(110px,0.82fr)_minmax(110px,0.7fr)_minmax(110px,0.82fr)_minmax(110px,0.7fr)_minmax(120px,0.72fr)_56px] items-center gap-3";
...
const SortLabel = ({
  ...
  return (
    ...
    >
      <span>{label}</span>
      <Icon
        className={`text-[10px] ${active ? "text-gray-700" : "text-gray-300"}`}
      />
    </button>
  );
};
...
const getOrderStatusStyle = (status) =>
  ORDER_STATUS_STYLES[status] || "bg-gray-100 text-gray-700 border-gray-200";
const getPaymentBadgeStyle = (status) =>
  PAYMENT_STATUS_STYLES[status] || "bg-gray-100 text-gray-700 border-gray-200";
...
const renderStatusBadge = (status) => (
  <span
    className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold ${getOrderStatusStyle(status)}`}
  >
...
const renderPaymentBadge = (payment) => {
  ...
  return (
    <div className="flex flex-col gap-1">
      <span
        className={`inline-flex w-fit items-center rounded-full border px-2.5 py-1 text-[11px] font-semibold ${getPaymentBadgeStyle(payment.status)}`}
      >
...
const OrderTable = ({
  ...
  return (
    <div className={`${loading ? "opacity-80" : ""}`}>
      <div
        className={`${GRID_CLASS} border-b border-gray-100 bg-gray-50 px-5 py-4 text-gray-400`}
      >
        <div className="text-[12px] font-semibold text-gray-400">
          Mã đơn hàng
        </div>
        <div className="text-[12px] font-semibold uppercase tracking-[0.08em] text-gray-400">
          Khách hàng
        </div>
        <div className="text-[12px] font-semibold uppercase tracking-[0.08em] text-gray-400">
          Sản phẩm
        ...
        orders.map((order) => {
          ...
          return (
            ...
            >
                ...
                {order.payment?.status === "PENDING" &&
                  order.payment?.method === "PAYOS" && (
                    <Button
                      ...
                      icon={
                        <SyncOutlined
                          spin={syncingPaymentId === order.payment.id}
                        />
                      }
              ...
              <div className="justify-self-start text-left text-[13px] text-gray-500">
