        ...
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-12">
          ...
          <div className="lg:col-span-5 grid grid-cols-2 gap-8">
            <div className="space-y-5">
              <h3 className="text-[14px] font-bold text-brand-cream font-serif">Danh mục</h3>
              <ul className="space-y-3">
                {topCategories.length > 0 ? (
                  topCategories.map((cat: any) => (
                    <li key={cat.id}>
                      <Link href={`/shop?categoryId=${cat.id}`} className="text-[13.5px] text-brand-taupe hover:text-brand-cream transition-colors duration-200">
                        {cat.name}
                      </Link>
                    </li>
                  ))
                  ...
                  ["Điện tử", "Thời trang", "Gia dụng", "Sức khỏe", "Flash Sale"].map((item) => (
                    <li key={item}>
                      <Link href="/shop" className="text-[13.5px] text-brand-taupe hover:text-brand-cream transition-colors duration-200">
                        {item}
                      </Link>
                    </li>
                  ))
            ...
            <div className="space-y-5">
              <h3 className="text-[14px] font-bold text-brand-cream font-serif">Hỗ trợ</h3>
              <ul className="space-y-3">
                {[
                  ...
                  <li key={item.name}>
                    <Link href={item.href} className="text-[13.5px] text-brand-taupe hover:text-brand-cream transition-colors duration-200">
                      {item.name}
                    </Link>
                  </li>
                ))}
