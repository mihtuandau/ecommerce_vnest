        ...
        <Table>
          <TableHeader className="bg-slate-50/50">
            <TableRow className="hover:bg-transparent border-slate-200">
              <TableHead className="font-semibold text-slate-700 py-4 h-11">
                Đơn hàng
              </TableHead>
              ...
              <TableHead className="text-right font-semibold text-slate-700 pr-6 h-11">
                Thao tác
              </TableHead>
            </TableRow>
          </TableHeader>
            ...
            {isLoading ? (
              ...
              returns.map((item) => (
                ...
                >
                  ...
                  <TableCell>
                    <span
                      className="text-[13px] text-slate-600 font-medium line-clamp-1 max-w-[200px]"
                      title={item.reason}
                    >
                      {item.reason}
                    </span>
                    ...
                    <div className="flex items-center justify-end gap-2">
                      {item.status === ReturnStatus.PENDING && (
                        <>
                          <button
                            onClick={() =>
                              handleUpdateStatus(item.id, ReturnStatus.APPROVED)
                            }
                          ...
                          <button
                            onClick={() =>
                              handleUpdateStatus(item.id, ReturnStatus.REJECTED)
                            }
                      ...
                      {item.status === ReturnStatus.APPROVED && (
                        <button
                          onClick={() =>
                            handleUpdateStatus(item.id, ReturnStatus.RETURNING)
                          }
                      ...
                      {item.status === ReturnStatus.RETURNING && (
                        <button
                          onClick={() =>
                            handleUpdateStatus(item.id, ReturnStatus.RECEIVED)
                          }
                      ...
                      {item.status === ReturnStatus.RECEIVED && (
                        <button
                          onClick={() =>
                            handleUpdateStatus(item.id, ReturnStatus.COMPLETED)
                          }
