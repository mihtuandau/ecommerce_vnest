...
const FAQS = [
  ...
  },
  {
    ...
    items: [
      { q: "Tôi có thể theo dõi đơn hàng của mình không?", a: "Có, ngay sau khi hàng được gửi đi, mã vận đơn sẽ được cập nhật trong phần Lịch sử đơn hàng của bạn để bạn có thể theo dõi trực tiếp." },
      { q: "Thời gian giao hàng mất bao lâu?", a: "Thông thường từ 1-2 ngày đối với khu vực nội thành và 2-4 ngày đối với các tỉnh thành khác." }
    ]
  },
  {
    category: "Chính sách đổi trả",
    items: [
      { q: "Sản phẩm bị lỗi tôi phải làm thế nào?", a: "Hãy liên hệ ngay hotline 1900 8888 trong vòng 30 ngày kể từ khi nhận hàng. Chúng tôi sẽ hướng dẫn bạn quy trình đổi mới sản phẩm miễn phí." }
    ]
  }
];
  ...
  return (
    <div className="max-w-[1000px] mx-auto py-12 md:py-20 px-4 sm:px-6">
      <header className="mb-12 border-b border-slate-100 pb-8">
        ...
        <p className="text-slate-500 text-sm">Giải đáp các thắc mắc phổ biến của khách hàng</p>
      </header>

      <div className="space-y-12">
        {FAQS.map((cat, idx) => (
          <section key={idx}>
            <h2 className="text-lg font-bold text-slate-900 mb-6 uppercase tracking-wider">0{idx + 1}. {cat.category}</h2>
            <div className="space-y-8">
              {cat.items.map((item, i) => (
                <div key={i}>
                  <p className="font-bold text-slate-800 mb-2">Q: {item.q}</p>
                  <p className="text-slate-600 pl-4 border-l-2 border-slate-100 italic font-normal">A: {item.a}</p>
                </div>
              ))}
            </div>
          </section>
