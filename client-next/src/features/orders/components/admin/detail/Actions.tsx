    ...
    <>
      ...
      <div className={cn(adminUI.card.base, "p-0 overflow-hidden")}>
        <div className="px-5 py-4 border-b border-slate-100 bg-white">
          ...
          >
            <CreditCard className="h-4 w-4 text-blue-500" fill="currentColor" /> Thanh
            toán
          </h3>
        </div>
        <div className="p-5 space-y-4">
          ...
          <div className="border-t border-slate-100 pt-3 flex justify-between items-start gap-4">
            ...
            <span className="text-[15px] font-bold font-serif text-amber-700 text-right">
              {(order.total || (order as any).totalAmount || 0).toLocaleString("vi-VN")}
              đ
            </span>
          </div>
            ...
            order.status !== OrderStatus.RETURNED && (
              <div className="border-t border-slate-100 pt-4 mt-1">
                <Button
                  className={cn(
                    adminUI.button.base,
                    "w-full bg-slate-900 hover:bg-slate-800 text-white rounded-lg h-9 text-[13px] font-semibold shadow-sm"
                  )}
                  onClick={() =>
                    orderAny.payment?.id &&
                    onUpdatePayment({
                      paymentId: String(orderAny.payment.id),
