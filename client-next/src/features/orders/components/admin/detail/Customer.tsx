...
export function Customer({ order }: CustomerProps) {
  ...
  const formattedTotalSpent = totalSpent >= 1000000 
    ...
    <>
      ...
      <div className={cn(adminUI.card.base, "p-0 overflow-hidden")}>
        <div className="px-5 py-4 border-b border-slate-100 bg-white flex justify-between items-center">
           <h3 className={cn(adminUI.typography.sectionTitle, "flex items-center gap-2")}>
             <User className="h-4 w-4 text-slate-700" fill="currentColor" /> Thông tin khách hàng
           </h3>
        ...
        <div className="p-5">
           <div className="flex items-center gap-3 mb-4">
             <div className="h-12 w-12 rounded-full bg-[#f3eae1] text-[#a47b59] flex items-center justify-center font-bold text-[15px]">
               {customerName !== "Khách vãng lai" ? customerName.substring(0, 2).toUpperCase() : "KV"}
             </div>
             <div>
           ...
           <div className="space-y-3">
             <div className="flex justify-between items-start gap-4">
               <span className="text-[13px] text-slate-500 whitespace-nowrap">Email</span>
             ...
             <div className="flex justify-between items-start gap-4">
               ...
               <span className="text-[13px] font-medium text-slate-800 text-right">{customerPhone}</span>
             </div>
             <div className="flex justify-between items-start gap-4">
               <span className="text-[13px] text-slate-500 whitespace-nowrap">Tổng đơn</span>
               <span className="text-[13px] font-medium text-slate-800 text-right">{totalOrders} đơn hàng</span>
             </div>
             <div className="flex justify-between items-start gap-4">
               <span className="text-[13px] text-slate-500 whitespace-nowrap">Tổng chi tiêu</span>
               <span className="text-[13px] font-medium text-amber-600 text-right">{formattedTotalSpent}</span>
             </div>
           </div>
        </div>
      </div>
      ...
      <div className={cn(adminUI.card.base, "p-0 overflow-hidden")}>
        <div className="px-5 py-4 border-b border-slate-100 bg-white">
           <h3 className={cn(adminUI.typography.sectionTitle, "flex items-center gap-2")}>
             <MapPin className="h-4 w-4 text-rose-500" fill="currentColor" /> Địa chỉ giao hàng
           </h3>
        </div>
        <div className="p-5 space-y-3">
           <div className="flex justify-between items-start gap-4">
             <span className="text-[13px] text-slate-500 whitespace-nowrap">Người nhận</span>
             <span className="text-[13px] font-medium text-slate-800 text-right">{customerName}</span>
           </div>
           <div className="flex justify-between items-start gap-4">
             <span className="text-[13px] text-slate-500 whitespace-nowrap">Địa chỉ</span>
             <span className="text-[13px] font-medium text-slate-800 text-right">{customerAddress}</span>
           </div>
           <div className="flex justify-between items-start gap-4">
             <span className="text-[13px] text-slate-500 whitespace-nowrap">Phương tiện</span>
             <span className="text-[13px] font-medium text-slate-800 text-right">Giao Hàng Nhanh</span>
           </div>
           <div className="flex justify-between items-start gap-4">
             <span className="text-[13px] text-slate-500 whitespace-nowrap">Mã vận đơn</span>
