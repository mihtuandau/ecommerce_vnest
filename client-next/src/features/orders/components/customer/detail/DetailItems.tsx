        ...
        <div className="p-[22px]">
          <table className="w-full border-collapse">
            ...
            <tbody className="divide-y divide-[#DDD6C8]">
              {orderItems?.map((item: any) => {
                ...
                const productName = item.productName || snap?.productName || live?.product?.name || "Sản phẩm LUXE";
                const brand = snap?.brandName || live?.product?.brand?.name || "LUXE Boutique";
                const productSlug = snap?.productSlug || live?.product?.slug || "#";
                const imageSrc = snap?.image || live?.images?.[0]?.url || live?.images?.[0] || live?.product?.images?.[0]?.url || live?.product?.images?.[0];
                ...
                return (
                  <tr key={item.id} className="group">
                    <td className="py-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="space-y-1">
                          ...
                          <Link href={`/shop/${productSlug}`} className="text-[14px] font-medium text-[#3D2B1A] hover:text-[#C4783A] transition-colors line-clamp-1 leading-snug">
                            {productName}
                          </Link>
                          {(color || size) && (
                            <div className="flex gap-1.5 pt-1">
                              {Boolean(color) && (
                                <span className="text-[11px] text-brand-taupe bg-brand-cream border border-brand-sand rounded-md px-2 py-0.5">Màu: {color}</span>
                              )}
                              {Boolean(size) && (
                                <span className="text-[11px] text-brand-taupe bg-brand-cream border border-brand-sand rounded-md px-2 py-0.5">Size: {size}</span>
                              )}
                            </div>
                          )}
                        </div>
                    ...
                    <td className="py-4 text-center text-[13.5px] text-brand-taupe font-sans tabular-nums">× {item.quantity}</td>
                    <td className="py-4 text-right">
                      <p className="text-[15px] font-bold text-brand-espresso font-sans tabular-nums">{formatCurrency(price)}</p>
                      {originalPrice > price && (
                        <p className="text-[12px] text-brand-taupe/80 line-through font-sans tabular-nums">{formatCurrency(originalPrice)}</p>
                      )}
                    </td>
          ...
          <div className="mt-4 pt-4 border-t border-brand-sand space-y-2.5 max-w-sm ml-auto font-sans">
            <div className="flex justify-between text-[13.5px]">
              ...
              <span className="text-brand-espresso font-medium tabular-nums">{formatCurrency(total - (shippingFee || 0) + (discountAmount || 0))}</span>
            </div>
            {discountAmount > 0 && (
              <div className="flex justify-between text-[13.5px]">
                <span className="text-brand-taupe">Giảm giá sản phẩm</span>
                <span className="text-rose-600 font-medium tabular-nums">− {formatCurrency(discountAmount)}</span>
              </div>
            )}
            <div className="flex justify-between text-[13.5px]">
              <span className="text-brand-taupe">Phí vận chuyển</span>
              <span className="text-emerald-600 font-bold tabular-nums">{shippingFee > 0 ? formatCurrency(shippingFee) : "Miễn phí"}</span>
            </div>
