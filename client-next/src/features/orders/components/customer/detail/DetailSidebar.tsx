...
export function DetailSidebar({
  ...
  const address =
    snap?.addressString ||
    [snap?.street, snap?.ward, snap?.district, snap?.province]
      .filter(Boolean)
      .join(", ") ||
    (typeof addressRelation === "object"
      ? [
          addressRelation?.street,
          addressRelation?.ward,
          addressRelation?.district,
          addressRelation?.province || addressRelation?.city,
        ]
          .filter(Boolean)
          .join(", ")
      : addressRelation) ||
  ...
  const recipientName = snap?.fullName || user?.name || "Khách hàng LUXE";
  const phone = snap?.phone || user?.phone || "Chưa cập nhật";
  const emailToDisplay = snap?.email || user?.email;
      ...
      <div className="bg-white border border-brand-sand rounded-2xl overflow-hidden shadow-sm">
        ...
        <div className="p-[18px] space-y-4">
          <div className="space-y-1">
            ...
            <p className="text-[13.5px] font-bold text-brand-espresso">
              {recipientName}
            </p>
          </div>
          ...
          <div className="space-y-1">
            ...
            <p className="text-[13.5px] text-brand-espresso">{phone}</p>
            {emailToDisplay && (
          ...
          <div className="space-y-1">
            ...
            <p className="text-[13.5px] text-brand-espresso leading-relaxed">
              {address}
            </p>
          </div>
        </div>
      ...
      <div className="bg-white border border-brand-sand rounded-2xl overflow-hidden shadow-sm">
        ...
        <div className="p-[18px] space-y-3">
          ...
          paymentMethod?.toLowerCase().includes("card") ? (
            <div className="bg-gradient-to-br from-brand-espresso to-brand-espresso/90 rounded-xl p-4 text-white space-y-4">
              <div className="flex justify-between items-start">
                <span className="text-[13px] font-bold tracking-widest opacity-80 uppercase">
                  LUXE Bank
                </span>
                <span className="text-xl font-bold italic opacity-60">VISA</span>
              </div>
              <div className="space-y-2">
                <p className="text-[14px] font-mono tracking-[0.2em] opacity-80">
                  •••• •••• •••• 1234
                </p>
            ...
            <div className="flex items-center gap-3 p-3 bg-brand-cream border border-brand-sand rounded-xl">
              <span className="p-2 bg-white rounded-lg border border-brand-sand">
                {paymentMethod === "COD" ? (
                  <Wallet size={20} className="text-brand-bronze" />
                ) : (
          ...
          <div className="pt-2 flex flex-col gap-1">
            ...
            <p className="text-[18px] font-bold text-brand-bronze font-sans tabular-nums">
          ...
          {shippingCode && (
            <div className="pt-2">
              ...
              <p className="text-[13.5px] font-bold text-brand-espresso font-mono">
                {shippingCode}
              </p>
            </div>
          )}
      ...
      <div className="bg-brand-cream border border-brand-sand rounded-2xl p-[18px] text-center space-y-4">
        <div className="space-y-1.5">
          ...
          <p className="text-[12.5px] text-brand-taupe leading-relaxed">
            Liên hệ đội ngũ LUXE nếu bạn gặp vấn đề với đơn hàng này.
          </p>
        </div>
        <div className="flex flex-col gap-2">
          <a
            href={`tel:${process.env.NEXT_PUBLIC_HOTLINE || "19008888"}`}
