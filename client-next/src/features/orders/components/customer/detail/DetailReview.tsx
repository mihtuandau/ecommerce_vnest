...
export function DetailReview({ orderItems, onReviewSubmit, isReviewed }: DetailReviewProps) {
  ...
  if (unreviewedItems.length === 0) {
    return (
      <div className="mt-8 p-10 bg-emerald-50 border border-emerald-100 rounded-[24px] text-center space-y-3">
        ...
        <p className="text-sm font-bold text-emerald-800">Cảm ơn bạn! Bạn đã đánh giá tất cả sản phẩm trong đơn này.</p>
      </div>
    );
  }
    ...
    <div className="mt-8 bg-white border border-[#DDD6C8] rounded-[24px] overflow-hidden shadow-sm font-sans-brand">
      <div className="px-8 py-6 border-b border-[#DDD6C8] flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-[10px] font-black text-[#3D2B1A] uppercase tracking-[0.25em]">Đánh giá sản phẩm</h3>
        </div>
      ...
      <div className="divide-y divide-[#DDD6C8]/30">
        {orderItems.map((item) => (
          <div key={item.id} className="p-8 grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
            <div className="md:col-span-1 flex items-center justify-center">
               <div className="h-20 w-16 relative rounded-lg overflow-hidden border border-[#DDD6C8] bg-[#F3EFE8]">
                  <Image 
                    src={getImageUrl(item.variant?.images?.[0] || item.variant?.product?.images?.[0])}
            ...
            <div className="md:col-span-7 space-y-3">
              <div>
                <p className="text-[10px] font-black text-[#8A7966] uppercase tracking-[0.2em] mb-1">{(item.variant?.product?.brand as any)?.name || "LUXE"}</p>
                <h4 className="text-[14px] font-bold text-[#3D2B1A] line-clamp-1">{item.productName}</h4>
                <p className="text-[11px] text-[#8A7966] mt-1">Màu: {item.variant?.color} · Size: {item.variant?.size}</p>
              </div>
                ...
                {[1, 2, 3, 4, 5].map((star) => (
                  ...
                  >
                    <Star
                      ...
                      className={cn(
                        "transition-colors",
                        (ratings[item.id] || 0) >= star ? "text-[#C4783A] fill-[#C4783A]" : "text-[#DDD6C8]"
                      )}
                    />
                  </button>
                ))}
            ...
            <div className="md:col-span-4 flex flex-col gap-3">
               ...
               />
               <Button 
                onClick={() => onReviewSubmit?.(item.id, ratings[item.id] || 0, comments[item.id] || "")}
