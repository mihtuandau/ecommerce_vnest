...
export function useAddressManagement() {
  ...
  const handleProvinceChange = useCallback(async (id: string) => {
    ...
    if (!id) {
      return { provinceId: id, provinceName, districtId: "", districtName: "", wardCode: "", wardName: "" };
    }
  ...
  const applySavedAddress = useCallback(async (addr: AddressOption): Promise<Partial<CheckoutFormData>> => {
    try {
