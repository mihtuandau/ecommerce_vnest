...
export function OrderStatusChart({ data, isLoading }: OrderStatusChartProps) {
  ...
  return (
    <Card className="border border-slate-200 shadow-none rounded-xl overflow-hidden bg-white h-full flex flex-col">
      <CardHeader>
        <CardTitle className="text-lg font-bold text-slate-900">Trạng thái đơn hàng</CardTitle>
        <CardDescription className="text-xs font-medium text-slate-500">Phân bổ theo trạng thái</CardDescription>
      </CardHeader>
        ...
        <div className="h-[400px] w-full flex flex-col">
          {isLoading ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-slate-400 gap-2">
              ...
              <p className="text-xs font-bold italic">Chưa có đơn hàng</p>
            </div>
          )}
        </div>
      </CardContent>
