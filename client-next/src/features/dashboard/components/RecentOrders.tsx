...
export function RecentOrders({ orders, isLoading }: RecentOrdersProps) {
  const recentOrders = (Array.isArray(orders) ? orders : orders as any)?.data || orders || [];

  return (
    <Card className="border border-slate-200 shadow-none rounded-xl overflow-hidden flex flex-col bg-white">
      <CardHeader className="flex flex-row items-center justify-between space-y-0">
        <div>
          <CardTitle className="text-lg font-bold text-slate-900">Đơn hàng mới</CardTitle>
          <CardDescription className="text-xs font-medium text-slate-500">Giao dịch gần đây nhất</CardDescription>
        </div>
        <Button variant="ghost" className="h-8 px-2 text-[10px] font-bold uppercase tracking-wider text-primary hover:bg-primary/5" asChild>
            ...
            recentOrders.slice(0, 5).map((order: any, index: number) => {
              ...
              return (
                ...
                <div className="text-right">
                  <p className="text-sm font-bold text-slate-900 mb-1">{formatCurrency(order.total)}</p>
                  <div className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md inline-block ${STATUS_MAP[order.status]?.bg} ${STATUS_MAP[order.status]?.color}`}>
