  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="icon" className="h-10 w-10 rounded-full text-brand-taupe relative">
          <Bell size={24} />
          {unreadCount > 0 && (
      ...
      <DropdownMenuContent align="end" className="w-[320px] rounded-[24px] p-2 bg-white shadow-2xl border border-brand-sand mt-2">
        <div className="px-4 py-3 border-b border-brand-sand/50 flex justify-between items-center">
          ...
          <Link href="/account?tab=notifications" className="text-[11px] font-bold text-brand-bronze hover:underline">Xem tất cả</Link>
        </div>
        <div className="max-h-[350px] overflow-y-auto">
          {notifications.length === 0 ? (
            <div className="py-10 text-center px-4">
              ...
              <p className="text-[12px] text-brand-taupe">Bạn không có thông báo mới nào</p>
            </div>
          ) : (
        ...
        <div className="p-2 border-t border-brand-sand/50">
          <Button variant="ghost" className="w-full justify-center text-[11px] font-bold text-brand-taupe hover:text-brand-espresso" asChild>
