  ...
  return (
    <section className="relative w-full flex flex-col lg:flex-row overflow-hidden bg-[#FAF8F4]">
        ...
        <div className="w-full max-w-lg space-y-8 animate-in fade-in slide-in-from-left-8 duration-1000">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-[4.5rem] leading-[1.05] text-[#3D2B1A] tracking-tight" style={{ fontFamily: "'Playfair Display', serif", fontWeight: 600 }}>
            {banner?.title ? (
              ...
              <>
                Phong cách <br />
                <em className="text-[#C4783A]" style={{ fontStyle: 'italic' }}>tỏa sáng</em><br />
                từng ngày
              </>
            )}
          </h1>

          <p className="text-[#8A7966] text-[15px] leading-relaxed max-w-sm">
            {banner?.description || "Khám phá hàng ngàn sản phẩm thời trang chính hãng, từ các thương hiệu trong nước đến quốc tế. Phong cách của bạn, sự lựa chọn của bạn."}
          </p>

          <div className="flex flex-col sm:flex-row items-center gap-4 pt-4">
            <Link
              href={banner?.link || "/shop"}
              className="w-full sm:w-auto inline-flex items-center justify-center bg-[#3D2B1A] text-[#FAF8F4] px-12 py-4 rounded-full text-[14px] font-bold hover:bg-[#C4783A] transition-all duration-300 shadow-xl shadow-[#3D2B1A]/10 active:scale-95"
            >
              Mua ngay
            </Link>
            <Link
            ...
            >
              Xem bộ sưu tập <ArrowRight className="w-4 h-4 text-[#8A7966]" />
            </Link>
          </div>
      ...
      <div className="w-full lg:w-1/2 relative min-h-[600px] lg:min-h-0 bg-[#E8E0D0] overflow-hidden flex items-center justify-center">
        ...
        {/* Floating Accent Card (The style you liked!) */}
        <div className="absolute bottom-8 right-8 lg:bottom-16 lg:right-16 z-20 w-64 md:w-72 bg-white/95 backdrop-blur-md p-5 rounded-3xl shadow-[0_32px_80px_rgba(61,43,26,0.15)] border border-white transform rotate-[-3deg] hover:rotate-0 transition-all duration-500 hover:shadow-3xl">
          <div className="bg-[#FAF8F4] h-48 rounded-2xl mb-4 relative overflow-hidden flex items-center justify-center border border-[#DDD6C8]/40">
            {banner && (banner.image || banner.imageUrl) ? (
              <Image src={(banner.image || banner.imageUrl) as string} alt="Banner Card" fill className="object-cover" />
            ) : (
              <Sparkles className="w-10 h-10 text-[#C4783A]/20" />
            )}
          </div>
          <p className="text-[10px] text-[#C4783A] font-bold uppercase tracking-[0.2em] mb-1">Mùa hè 2025</p>
