...
export const ProductCard = React.memo(function ProductCard({
  ...
  const originalPriceVal =
    product.variants?.[0]?.originalPrice || product.originalPrice;
        ...
        >
          <div className="relative h-40 w-40 md:h-48 md:w-48 bg-brand-ivory flex-shrink-0 flex items-center justify-center p-4">
            <Image
              ...
              alt={product.name}
              fill
              className="object-contain p-6 transition-transform duration-700 ease-out group-hover:scale-105 mix-blend-multiply"
          ...
          <div className="flex-1 p-5 md:p-6 flex flex-col justify-between">
            <div className="space-y-2">
              <div className="flex justify-between items-start">
                <div className="space-y-1">
                  <p className="text-[10px] font-bold text-brand-taupe uppercase tracking-[0.12em]">
                    {product.category?.name || "Luxurious"}
                  </p>
                  <h3 className="text-[16px] md:text-[18px] font-medium text-primary leading-tight hover:text-brand-bronze transition-colors">
                    {product.name}
                  </h3>
                </div>
                <Button
                  ...
                  size="icon"
                  className={cn(
                    "relative z-30 h-8 w-8 border transition-all duration-300",
                    isFavorite
                      ? "bg-brand-bronze border-brand-bronze text-white hover:bg-brand-bronze/95 hover:text-white"
                      : "bg-white border-brand-sand text-brand-taupe hover:bg-brand-cream hover:text-brand-bronze"
                  )}
                >
                  <Heart
                    size={14}
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-[12px] text-brand-taupe/40">
            ...
            <div className="flex items-center justify-between pt-4">
              <Price amount={price} originalAmount={originalPrice || undefined} showBadge size="xl" />
              <Button
      ...
      >
        <div className="relative h-[280px] w-full bg-brand-ivory overflow-hidden flex items-center justify-center">
          <Image
            ...
            alt={product.name}
            fill
            className="object-contain p-8 transition-transform duration-700 ease-out group-hover:scale-105 mix-blend-multiply"
            sizes="(max-width: 768px) 50vw, 25vw"
          />
          <div className="absolute top-4 left-4 z-30 flex flex-col gap-1.5">
            {discountPercent > 0 && (
              <span className="bg-brand-bronze text-white text-[9px] font-bold px-2 py-1 rounded uppercase tracking-wider">
                -{discountPercent}%
              </span>
            )}
            {product.isNew && (
              <span className="bg-primary text-white text-[9px] font-bold px-2 py-1 rounded uppercase tracking-wider">
                Mới
              </span>
            )}
          </div>
          <Button
            ...
            size="icon"
            className={cn(
              "absolute top-4 right-4 z-30 h-8 w-8 border transition-all duration-300",
              isFavorite
                ? "bg-brand-bronze border-brand-bronze text-white hover:bg-brand-bronze/95 hover:text-white"
                : "bg-white/80 backdrop-blur-sm border-brand-sand text-brand-taupe hover:bg-white hover:text-brand-bronze"
            )}
          >
            <Heart
              size={14}
        ...
        <div className="p-4 md:p-5 flex flex-col flex-1">
          <div className="mb-2">
            <p className="text-[11px] font-bold text-brand-taupe uppercase tracking-[0.1em] mb-1">
          ...
          <div className="flex items-center justify-between gap-4 mb-4">
            <div className="flex items-center gap-2 shrink-0">
              ...
              <span className="text-[12px] text-brand-taupe/80 font-medium">
                ({product.reviewCount || 0})
              </span>
            </div>
