    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-white border border-brand-sand rounded-2xl overflow-hidden shadow-[0_4px_20px_rgba(61,43,26,0.03)]">
        <div className="px-6 py-5 border-b border-brand-sand/50 flex items-center justify-between">
          <div>
            ...
            <p className="text-[12px] text-brand-taupe mt-0.5">Cập nhật ảnh đại diện và thông tin cơ bản của bạn</p>
          </div>
        </div>
        ...
        <div className="p-6 lg:p-8 space-y-10">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6 pb-8 border-b border-brand-sand/40">
            ...
            <div className="text-center sm:text-left space-y-3">
              ...
              <p className="text-[12px] text-brand-taupe leading-relaxed">JPG, PNG tối đa 5MB. Kích thước đề nghị 400×400px.</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-[12.5px] font-medium text-brand-espresso">Họ và tên <span className="text-red-500">*</span></label>
              <Input required value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="h-11 rounded-xl border-brand-sand focus:border-brand-espresso transition-all bg-white" />
            </div>
            <div className="space-y-2">
              <label className="text-[12.5px] font-medium text-brand-espresso">Tên hiển thị</label>
              <Input value={form.name} readOnly className="h-11 rounded-xl border-brand-sand focus:border-brand-espresso transition-all bg-brand-ivory/20 text-brand-taupe cursor-default" />
            </div>
            <div className="space-y-2">
              <label className="text-[12.5px] font-medium text-brand-espresso">Ngày sinh</label>
              <Input 
                type="date" 
            ...
            <div className="space-y-2">
              <label className="text-[12.5px] font-medium text-brand-espresso">Giới tính</label>
        ...
        <div className="px-6 py-4 bg-brand-ivory/20 border-t border-brand-sand/50 flex justify-end gap-3">
          <Button variant="ghost" className="rounded-full h-10 px-6 text-[13px] font-medium text-brand-taupe">Huỷ thay đổi</Button>
          <Button onClick={handleUpdateProfile} className="rounded-full h-10 px-8 text-[13px] font-bold bg-brand-espresso text-white hover:bg-brand-espresso/90 shadow-lg shadow-brand-espresso/10 transition-all">
      ...
      <div className="bg-white border border-brand-sand rounded-2xl overflow-hidden shadow-[0_4px_20px_rgba(61,43,26,0.03)]">
        ...
        <div className="p-6 lg:p-8 grid grid-cols-1 sm:grid-cols-2 gap-8">
          <div className="space-y-2">
            <label className="text-[12.5px] font-medium text-brand-espresso">Email</label>
            <Input value={user.email} disabled className="h-11 rounded-xl border-brand-sand bg-brand-ivory/20 text-brand-taupe" />
            <p className="text-[11px] text-emerald-600 font-medium ml-1">✓ Email đã xác minh</p>
          </div>
          <div className="space-y-2">
            <label className="text-[12.5px] font-medium text-brand-espresso">Số điện thoại</label>
            <div className="flex gap-2">
              <Input value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} className="h-11 rounded-xl border-brand-sand focus:border-brand-espresso bg-white flex-1" />
              <Button variant="outline" className="h-11 rounded-xl border-brand-sand text-[12px] font-bold text-brand-taupe hover:text-brand-espresso whitespace-nowrap">Xác minh</Button>
            </div>
            <p className="text-[11px] text-amber-600 font-medium ml-1">⚠ Chưa xác minh SĐT</p>
          </div>
        </div>
      </div>
    </div>
