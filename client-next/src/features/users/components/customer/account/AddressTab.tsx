...
import { MapPin, Plus, Building2, Home, Phone, Star, Trash2 } from "lucide-react";
...
export function AddressTab() {
  ...
  const handleAddAddress = async (e: React.FormEvent) => {
    ...
    const provinceName = provinces.find(
      (p) => String(p.ProvinceID) === String(newAddress.provinceId)
    )?.ProvinceName;
    const districtName = districts.find(
      (d) => String(d.DistrictID) === String(newAddress.districtId)
    )?.DistrictName;
    const wardName = wards.find(
      (w) => String(w.WardCode) === String(newAddress.wardCode)
    )?.WardName;
  ...
  const handleEditClick = (address: Address) => {
    ...
    if (address.provinceCode) {
      shippingApi
        .getDistricts(Number(address.provinceCode))
        .then((res) => setDistricts(res.data || []));
    }
    if (address.districtCode) {
      shippingApi
        .getWards(Number(address.districtCode))
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="bg-white border border-brand-sand rounded-2xl overflow-hidden shadow-[0_4px_20px_rgba(61,43,26,0.03)]">
        <div className="px-6 py-5 border-b border-brand-sand/50 flex items-center justify-between">
          ...
          <Button
            ...
            className="rounded-full h-10 px-6 text-[12.5px] font-bold bg-brand-espresso text-white hover:bg-brand-espresso/90 shadow-lg shadow-brand-espresso/10 transition-all"
          >
        ...
        <div className="p-6 lg:p-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              (addressData?.addresses || []).map((address: Address) => (
                <div
                  ...
                  className={cn(
                    ...
                    address.isDefault
                      ? "border-brand-espresso bg-brand-cream/10"
                      : "border-brand-sand hover:border-brand-taupe shadow-sm"
                  )}
                >
                  <div className="flex justify-between items-start mb-3">
                    <span
                      className={cn(
                        ...
                        address.street.toLowerCase().includes("văn phòng") ||
                          address.fullName.toLowerCase().includes("vp")
                    ...
                    {address.isDefault && (
                      <span className="text-[10px] font-bold text-white bg-brand-espresso px-2.5 py-1 rounded-md tracking-wider">
                        Mặc định
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2 pt-4 border-t border-brand-sand/40">
                    <Button
                      ...
                      variant="outline"
                      className="flex-1 h-9 rounded-lg border-brand-sand text-[12px] font-bold text-brand-taupe hover:text-brand-espresso"
                    >
                      Sửa
                    </Button>
                    {!address.isDefault && (
                    ...
                    <Button
                      ...
                      variant="outline"
                      className="h-9 w-9 rounded-lg border-brand-sand text-red-400 hover:text-red-600 hover:bg-red-50 p-0"
                    >
                      <Trash2 size={14} />
                    </Button>
                  </div>
            ...
            >
              ...
              <span className="text-[13.5px] font-bold text-brand-taupe group-hover:text-brand-espresso">
                Thêm địa chỉ mới
              </span>
            </button>
          </div>
          ...
          <form onSubmit={handleAddAddress}>
            <div className="p-10 space-y-6">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                >
                  <SelectTrigger className="h-11 rounded-xl border-brand-sand text-[13px]">
              ...
              <div className="flex items-center gap-3 bg-brand-ivory/20 p-4 rounded-xl border border-brand-sand">
                ...
                <label
                  htmlFor="isDefault"
                  className="text-brand-espresso cursor-pointer font-bold text-[11px] tracking-wider select-none"
                >
                  Đặt làm mặc định
                </label>
              </div>
            ...
            <DialogFooter className="px-10 py-6 bg-brand-ivory/30 border-t border-brand-sand/50 gap-4">
              <Button
                ...
                className="font-bold text-[12px] text-brand-taupe"
              >
                Hủy
              </Button>
              <Button
                ...
                className="bg-brand-espresso hover:bg-brand-espresso/90 text-white font-bold px-10 rounded-full h-11 text-[12px] tracking-wider shadow-lg shadow-brand-espresso/10"
              >
      ...
      >
        <DialogContent className="sm:max-w-[550px] rounded-[32px] p-0 overflow-hidden border-none shadow-2xl">
          <DialogHeader className="px-10 py-8 bg-brand-ivory/30 border-b border-brand-sand/50">
            <DialogTitle className="text-lg font-bold text-brand-espresso font-serif flex items-center gap-3">
              <MapPin size={20} className="text-brand-bronze" /> Chỉnh sửa địa chỉ nhận
              hàng
            </DialogTitle>
          </DialogHeader>
