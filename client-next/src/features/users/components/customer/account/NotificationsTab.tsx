...
export function NotificationsTab() {
  ...
  const { data: notificationsData, isLoading: isLoadingNotifs } = useNotifications({ limit: 10, page });
    ...
    if (notificationsData?.data) {
      if (page === 1) {
        ...
        setAllNotifications(prev => {
          ...
          const newItems = notificationsData.data.filter((n: any) => !existingIds.has(n.id));
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'ORDER_STATUS': return <ShoppingBag size={18} className="text-blue-500" />;
      case 'SECURITY': return <ShieldAlert size={18} className="text-red-500" />;
      case 'PROMOTION': return <CheckCircle2 size={18} className="text-emerald-500" />;
      default: return <Bell size={18} className="text-brand-bronze" />;
    }
  };

  const notificationOptions = [
    { key: "orderStatus", title: "Trạng thái đơn hàng", desc: "Cập nhật khi đơn hàng đang giao hoặc đã hoàn tất" },
    { key: "promotions", title: "Khuyến mãi & Ưu đãi", desc: "Thông báo về mã giảm giá và các sự kiện Flash Sale" },
    { key: "newsletter", title: "Bản tin LUXE", desc: "Cập nhật xu hướng thời trang và bộ sưu tập mới" },
    { key: "security", title: "Bảo mật tài khoản", desc: "Thông báo khi có đăng nhập từ thiết bị lạ" },
  ];
        ...
        {activeSubTab === "inbox" ? (
          ...
          <div className="bg-white border border-brand-sand rounded-[24px] overflow-hidden shadow-sm">
            <div className="px-6 py-4 border-b border-brand-sand/50 flex items-center justify-between">
              <h3 className="text-[15px] font-bold text-brand-espresso">Thông báo của tôi</h3>
              {unreadCount > 0 && (
                <button 
            ...
            <div className="divide-y divide-brand-sand/30">
              {allNotifications.length === 0 ? (
                <div className="py-24 text-center px-4">
                  ...
                  <p className="text-[14px] font-medium text-brand-taupe">Hộp thư hiện đang trống</p>
                </div>
              ) : (
                <>
                  {allNotifications.map((n: any) => (
                    ...
                    >
                      <div className={cn(
                        ...
                        !n.isRead ? "bg-white border-brand-sand" : "bg-brand-ivory/50 border-transparent"
                      )}>
                        {getIcon(n.type)}
                      </div>
                      <div className="flex-1">
                        <div className="flex justify-between items-start mb-1">
                          <p className={cn(
                            ...
                            !n.isRead ? "font-bold text-brand-espresso" : "font-medium text-brand-taupe"
                          )}>
                            {n.title}
                          </p>
                  ...
                  {meta && page < meta.totalPages && (
                    <div className="p-6 text-center border-t border-brand-sand/30 bg-brand-ivory/5">
                      ...
                      >
                        {isLoadingNotifs ? <Spinner size="sm" /> : "Xem thêm thông báo cũ"}
                      </Button>
                    </div>
                  )}
                </>
          ...
          <div className="max-w-2xl">
            <div className="bg-white border border-brand-sand rounded-[24px] overflow-hidden shadow-sm">
              <div className="p-6 border-b border-brand-sand/50">
                <h3 className="text-[15px] font-bold text-brand-espresso">Cấu hình nhận tin</h3>
                <p className="text-[12px] text-brand-taupe mt-1">Chọn loại thông báo bạn muốn nhận qua ứng dụng</p>
              </div>
              <div className="divide-y divide-brand-sand/30">
                {notificationOptions.map((n) => (
                  ...
                  >
                    <div className="pr-8">
                      <p className="text-[14px] font-bold text-brand-espresso">{n.title}</p>
                    ...
                    <div className={cn(
                      ...
                      localSettings[n.key as keyof typeof localSettings] ? "bg-emerald-500" : "bg-brand-sand"
                    )}>
                      <div className={cn(
                        "absolute top-1 left-1 w-4 h-4 rounded-full bg-white transition-transform duration-300 shadow-sm",
                        localSettings[n.key as keyof typeof localSettings] ? "translate-x-5" : ""
                      )} />
                    </div>
                  </div>
                ))}
              ...
              <div className="p-6 bg-brand-ivory/10 border-t border-brand-sand/50 flex justify-end">
                ...
                >
                  {updateSettings.isPending ? <Spinner size="sm" variant="white" /> : "Cập nhật thay đổi"}
                </Button>
              </div>
            </div>
          </div>
