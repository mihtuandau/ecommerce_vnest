...
export function ReviewsTab() {
  ...
  const reviews = React.useMemo(() => {
    ...
    if (typeof possibleList === "object" && possibleList !== null) {
      const nestedList =
        possibleList.reviews || possibleList.data || possibleList.items || [];
          ...
          {isLoadingReviews ? (
            ...
            <div className="py-12 text-center space-y-4">
              ...
              <p className="text-sm text-brand-taupe max-w-sm mx-auto">
                Mua sắm và chia sẻ trải nghiệm của bạn để nhận thêm điểm thưởng tích
                lũy!
              </p>
              <Button
            ...
            <div className="grid grid-cols-1 gap-6">
              {reviews.map((review: any) => (
                <div
                  key={review.id}
                  className="bg-brand-cream/10 border border-brand-sand rounded-2xl p-6 transition-all hover:border-brand-taupe shadow-sm group"
                >
                  ...
                  <div className="bg-white/50 rounded-xl p-4 border border-brand-sand/50">
                    <p className="text-sm text-brand-espresso leading-relaxed italic">
                      "{review.comment}"
                    </p>
                  </div>
                  ...
                  {review.images && review.images.length > 0 && (
                    <div className="flex gap-2 mt-4 overflow-x-auto pb-2 no-scrollbar">
                      {review.images.map((img: any, idx: number) => (
                        <div
                          key={img.id || idx}
                          className="relative h-16 w-16 flex-shrink-0 group cursor-zoom-in"
                        >
                          <Image
                            src={img.url}
                            alt="Review"
