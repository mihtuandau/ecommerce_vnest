...
export function CustomerChatView({ onClose }: CustomerChatProps) {
  ...
  const isImageMessage = (msgText: string) => {
    return (
      ...
      (msgText.match(/\.(jpeg|jpg|gif|png|webp|svg)/i) || msgText.includes("/uploads/"))
    );
  };
  ...
  const handleFileChange = async (
    ...
    try {
      setIsUploading(true);
      toast.loading(isImageOnly ? "Đang tải hình ảnh lên..." : "Đang tải tệp lên...", {
        id: "customer-uploading-chat",
      });

      const url = await productsApi.uploadImage(file);

      toast.success(
        isImageOnly ? "Tải ảnh lên thành công!" : "Tải tệp lên thành công!",
        { id: "customer-uploading-chat" }
      );
  ...
  return (
    <div className="flex flex-col h-full w-full bg-white overflow-hidden font-sans border-0">
      ...
      >
        ...
        {messages
          .filter((m) => !m.message.startsWith("[INTERNAL]"))
          .map((m, i, filteredList) => {
            const isUser = m.senderId === user.id;
            const showTime =
              i === filteredList.length - 1 ||
              dayjs(filteredList[i + 1].createdAt).diff(dayjs(m.createdAt), "minute") >
                5;
          ...
          >
            <Send
              ...
              className={cn(
                ...
                inputText.trim() || pendingAttachment ? "translate-x-0.5" : ""
              )}
            />
          </button>
        </div>
