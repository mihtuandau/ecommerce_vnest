  ...
  return (
    <div className="flex-1 p-8 space-y-6 overflow-y-auto h-full [&::-webkit-scrollbar]:w-[4px] [&::-webkit-scrollbar-thumb]:bg-[#E8E0D0]">
      <div>
        ...
        <p className="text-[12px] text-[#8A7966] mt-1.5 leading-relaxed font-normal">Chúng tôi cam kết đảm bảo tối đa quyền lợi của khách hàng khi đặt mua sản phẩm tại LUXE.</p>
      </div>

      {/* Steps Process */}
      <div className="p-6 rounded-2xl bg-[#FAF8F4]/30 border border-[#DDD6C8]/50 shadow-3xs">
        <span className="text-[10px] text-[#8A7966] uppercase tracking-wider block mb-4">Các bước yêu cầu đổi trả</span>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative">
          {[
            { step: "01", title: "Liên hệ Hotline/Chat", desc: "Thông báo mã đơn hàng & lý do yêu cầu đổi trả cho tư vấn viên." },
            { step: "02", title: "Đóng gói sản phẩm", desc: "Gửi sản phẩm còn nguyên nhãn mác, tem niêm phong ban đầu." },
            { step: "03", title: "Kiểm tra & Hoàn tiền", desc: "Nhận tiền hoàn hoặc hàng đổi mới trong vòng 3 - 5 ngày làm việc." }
          ].map((step, idx) => (
            <div key={idx} className="relative z-10">
              <div className="w-8 h-8 rounded-lg bg-[#3D2B1A] text-white flex items-center justify-center text-[11px] shadow-3xs mb-3 font-medium">{step.step}</div>
              <h4 className="text-[12.5px] font-medium text-[#3D2B1A]">{step.title}</h4>
              <p className="text-[11px] text-[#8A7966] mt-1.5 leading-relaxed font-normal">{step.desc}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-3">
        <h4 className="text-[11px] text-[#8A7966] uppercase tracking-wider font-semibold">Câu hỏi thường gặp về đổi trả</h4>
        {returnFaqs.map(faq => {
