...
export function MessageBubble({
  ...
  if (isInternal) {
    return (
      <div
        key={m.id || i}
        className="w-full flex justify-center my-2 animate-in fade-in duration-200"
      >
  ...
  const senderRole =
    ROLE_CONFIG[m.sender?.role as Role]?.label || m.sender?.role || "Nhân viên";
  const initials =
    senderName
      .split(" ")
      .map((n: string) => n[0])
  ...
  return (
    <div
      key={m.id || i}
      className={cn("flex gap-3.5 items-end", isUser ? "flex-row-reverse" : "")}
    >
      {!isUser && (
        <div
          className={cn(
      ...
      >
        ...
        <div
          className={cn("flex items-center gap-1.5 px-1", isUser ? "justify-end" : "")}
        >
          <span className="text-[9px] font-medium text-slate-400">
            {dayjs(m.createdAt).format("HH:mm")}
          </span>
          {isUser && (
            <span className="text-emerald-600 text-[9px] font-bold">Đã gửi</span>
          )}
        </div>
      </div>
    </div>
