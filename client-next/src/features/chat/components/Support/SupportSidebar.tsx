          ...
          staffMembers.length === 0 ? (
            ...
            staffMembers
              ...
              .map((s: any) => {
                ...
                const initials = s.name?.split(" ").map((n: string) => n[0]).join("").slice(0, 2).toUpperCase() || "NV";
                return (
                  <div 
                    key={s.id}
                    ...
                    className={cn(
                      ...
                      isSelected ? "bg-amber-50/40 border-l-[3px] border-l-amber-500" : "hover:bg-slate-50/55"
                    )}
                  >
                    <div className={cn(
                      "w-10 h-10 rounded-xl flex items-center justify-center font-bold text-xs shrink-0 border shadow-xs",
                      isSelected ? "bg-amber-100 text-amber-750 border-amber-200" : "bg-slate-100 text-slate-550 border-slate-200"
                    )}>
                      {initials}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-0.5">
                        <span className={cn("text-[13px] font-semibold truncate", isSelected ? "text-amber-800" : "text-slate-800")}>
                          {s.name}
                        </span>
            ...
            return (
              <div 
                ...
                className={cn(
                  ...
                  isActive ? "bg-slate-50/70 border-l-[3px] border-l-indigo-600" : "hover:bg-slate-50/50"
                )}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-0.5">
                    <span className={cn("text-[13px] font-semibold truncate", isActive ? "text-indigo-650" : "text-slate-800")}>
                      {customer?.name || "Khách hàng"}
                    </span>
                    <span className="text-[10px] text-slate-400 font-medium whitespace-nowrap">{dayjs(lastMsg?.createdAt).fromNow(true)}</span>
                  </div>
                  <div className="text-xs text-slate-500 truncate mb-1 leading-relaxed">{lastMsg?.message}</div>
