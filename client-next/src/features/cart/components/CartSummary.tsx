  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-white rounded-[24px] p-6 border border-brand-sand shadow-sm">
        <h2 className="text-lg font-bold text-brand-espresso mb-6 font-serif">Tóm tắt đơn hàng</h2>
        
        <div className="space-y-4">
          <div className="flex justify-between items-center text-[14px]">
            <span className="text-brand-taupe font-medium">Tạm tính ({selectedCount} sản phẩm)</span>
            <span className="font-bold text-brand-espresso tabular-nums">{formatCurrency(selectedTotalPrice)}</span>
          </div>

          <div className="flex justify-between items-center pt-3 border-t border-brand-ivory">
            <span className="text-brand-taupe font-medium text-[14px]">Mã giảm giá</span>
            {!appliedDiscount ? (
              <button 
              ...
              <div className="flex items-center gap-2">
                <span className="text-[13px] font-bold text-brand-bronze uppercase tracking-wider">{appliedDiscount.code}</span>
          ...
          {appliedDiscount && discountAmount > 0 && (
            <div className="flex justify-between items-center text-[14px] animate-in fade-in slide-in-from-top-1 duration-300">
              ...
              <span className="font-bold text-brand-bronze tabular-nums">- {formatCurrency(discountAmount)}</span>
            </div>
          )}

          <div className="flex justify-between items-center pb-3">
            <span className="text-brand-taupe font-medium text-[14px]">Phí vận chuyển</span>
          ...
          <div className="pt-5 border-t border-brand-ivory flex flex-col gap-1">
            <div className="flex justify-between items-end">
              <span className="text-base font-bold text-brand-espresso font-serif">Tổng cộng</span>
          ...
          <div className="pt-2 flex flex-wrap items-center justify-center gap-2">
            {["VNPAY", "MoMo", "Visa", "COD"].map(pay => (
              <div key={pay} className="px-2 py-1 rounded bg-brand-cream text-[9px] font-bold text-brand-taupe border border-brand-sand uppercase tracking-wider">
                {pay}
              </div>
            ))}
          </div>
      ...
      <div className="bg-white rounded-[24px] p-5 border border-brand-sand shadow-sm space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="text-[12px] font-bold text-brand-espresso">30 ngày đổi trả</h4>
