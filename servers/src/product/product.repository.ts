...
export class ProductRepository {
  ...
  async createImages(images: any[]) {
    return this.prisma.productImage.createMany({ data: images });
  }
  async createVariantImages(images: any[]) {
    return this.prisma.variantImage.createMany({ data: images });
  }
  async deleteImages(ids: number[]) {
    return this.prisma.productImage.deleteMany({ where: { id: { in: ids } } });
  }
  async deleteVariantImages(ids: number[]) {
    return this.prisma.variantImage.deleteMany({ where: { id: { in: ids } } });
  }
  ...
  async updateThumbnailStatus(pId: number, isThumbnail: boolean) {
    return this.prisma.productImage.updateMany({
      where: { productId: pId },
      data: { isThumbnail },
    });
  }
  async updateVariantPrimaryStatus(vId: number, isPrimary: boolean) {
  ...
  async findVariantImageById(id: number) {
    return this.prisma.variantImage.findUnique({ where: { id } });
  }
  ...
  async incrementSoldCount(id: number, qty: number) {
    return this.prisma.product.update({
      where: { id },
  ...
  async getPriceRange() {
    ...
    return {
      minPrice: res._min.price || 0,
      maxPrice: res._max.price || 100000000,
    };
  }
}
