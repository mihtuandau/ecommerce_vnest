export const serializePayment = (payment: any) => {
  ...
  if (
    payment.order?.shippingSnapshot &&
    typeof payment.order.shippingSnapshot === 'object'
  ) {
    guestNameFromSnapshot =
      payment.order.shippingSnapshot.fullName || 'Khách vãng lai';
  }
  ...
  return {
    ...
    order: payment.order
      ? {
          ...
          customerPhone:
            payment.order.user?.phone || payment.order.guestPhone || 'N/A',
        }
      : undefined,
  };
};
