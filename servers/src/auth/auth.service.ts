...
export class AuthService {
  ...
  async register(dto: any) {
    ...
    if (existing?.status === UserStatus.SUSPENDED) {
      throw new BadRequestException(
        'Tài khoản này đã bị vô hiệu hóa. Vui lòng liên hệ hỗ trợ để biết thêm chi tiết.',
      );
    }
    ...
    if (existing) {
      ...
      return {
        message: 'Bạn chưa xác thực tài khoản. Chúng tôi đã gửi lại mã OTP.',
        email,
      };
    } else {
      // Create new user
  ...
  async resendOtp(email: string) {
    ...
    if (u.status !== UserStatus.PENDING) {
      throw new BadRequestException(
        'Chỉ có thể gửi lại mã cho tài khoản chưa xác thực',
      );
    }
    const otp = crypto.randomInt(100000, 1000000).toString();
  ...
  async login(
    ...
    if (u.twoFactorEnabled) {
      ...
      return {
        ...
        email: u.email,
        message:
          'Tài khoản đã bật bảo mật 2 lớp. Vui lòng nhập mã xác thực gửi tới email.',
      };
    }
    ...
    return {
      accessToken: this.jwtService.sign(common, {
      ...
      user: {
        ...safeUser,
        permissions: await this.userService.getPermissionsByRole(u.role),
      },
    };
  }
  ...
  async refreshAccessToken(refreshToken: string) {
    try {
      ...
      const jti = crypto.randomBytes(16).toString('hex');
      const common = { sub: u.id, email: u.email, role: u.role };
      const newAccessToken = this.jwtService.sign(common, {
        secret: process.env.JWT_SECRET,
        expiresIn: '2h',
      });
  ...
  private async checkNewDevice(
    ...
    console.log(
      `[SecurityCheck] Checking login for User: ${user.email} | IP: ${ip}`,
    );

    if (!ip || !userAgent) {
      console.log(
        `[SecurityCheck] Missing IP or UserAgent. IP: ${ip}, UA: ${userAgent}`,
      );
      return;
    }
    ...
    console.log(
      `[SecurityCheck] Found ${logs.length} previous logins from this IP.`,
    );

    const isNewDevice = !logs.some(
      (log) => (log.newData as any)?.userAgent === userAgent,
    );
    ...
    if (isNewDevice) {
      ...
      const isSecurityEnabled = settings?.security !== false;
      console.log(
        `[SecurityCheck] Security Notifications Enabled: ${isSecurityEnabled}`,
      );

      if (isSecurityEnabled) {
        console.log(
          `[SecurityCheck] Creating SECURITY notification for user ${user.id}`,
        );
        // Create notification
  ...
  async updateRolePermissions(role: string, pIds: number[]) {
    return this.userService.updateRolePermissions(role, pIds);
  }
