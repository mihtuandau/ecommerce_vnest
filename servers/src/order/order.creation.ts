...
export class OrderCreation {
  ...
  async create(
    ...
    const { priceMap, discountId: autoDiscountId } =
    ...
    if (useAutoApply && autoDiscountId) {
      finalDiscount = await this.prisma.discount.findUnique({
        where: { id: autoDiscountId },
      });
      if (manualDiscount && dto.discountCode) {
        ...
        this.logger.log(
          `[OrderCreation] Flash Sale (${autoApplySaving}) > Voucher "${dto.discountCode}" (${manualCodeSaving}). Auto-applied Flash Sale.`,
        );
      }
    ...
    const isPOS =
      dto.status === 'DELIVERED' || dto.shippingAddress === 'Mua tại quầy';
    ...
    try {
      ...
      if (isOnlinePayment && !isPOS) {
        throw new BadRequestException(
          'Không thể tính phí vận chuyển qua GHN. Vui lòng thử lại.',
        );
      }
      ...
      if (!isPOS) {
        this.logger.warn(
          'GHN fee calculation failed, using fallback:',
          error.message,
        );
      }
    }
    ...
    if (totals.totalItems >= systemSettings.freeShippingThreshold) {
      ...
      totals.discountedTotal = Math.max(
        0,
        totals.total - totals.discountAmount,
      );
    }
    ...
    try {
      ...
      this.logger.debug(
        `[OrderCreation] Created payment for ${order.orderCode}: ${payment ? 'OK' : 'NULL'}`,
      );
      if (payment) {
        this.logger.debug(
          `[OrderCreation] Link detail: ${payment.paymentLink ? 'FOUND' : 'NOT FOUND'}`,
        );
      }

      if (dto.paymentMethod === 'VNPAY' && (!payment || !payment.paymentLink)) {
        throw new Error(
          'Hệ thống không thể tạo liên kết thanh toán VNPay. Vui lòng kiểm tra lại cấu hình.',
        );
      }
    } catch (error) {
      this.logger.error(
        `Failed to create payment for order ${order.id}. Initiating rollback. Error: ${error.message}`,
      );
      ...
      throw new BadRequestException(
        error.message || 'Lỗi hệ thống khi tạo giao dịch thanh toán.',
      );
    }

    await this.clearUserCartIfNeeded(userId, dto);
    if (userId) await this.cacheService.deleteUserOrderCaches(userId);
    await this.notifyAdminsAboutNewOrder(
      order,
      systemSettings.orderNotification,
    );
  ...
  private async prepareItemsFromDto(
    ...
    return items.map((item) => {
      ...
      const brandName =
        (variant.product as any)?.brand?.name || 'LUXE Boutique';
      const variantImage =
  ...
  private async prepareItemsFromCart(userId: number) {
    ...
    if (invalidItems.length > 0) {
      const names = invalidItems
        .map(
          (it: any) => it.variant?.product?.name || `variant#${it.variantId}`,
        )
        .join(', ');
      throw new BadRequestException(
    ...
    return cart.cartItems.map((item) => {
      const v = item.variant;
      const brandName = v?.product?.brand?.name || 'LUXE Boutique';
      const variantImage =
        v?.images?.[0]?.url || v?.product?.images?.[0]?.url || null;
      ...
      return {
        ...
        variantSnapshot: {
          ...
          brandName: brandName,
          originalPrice:
            v?.originalPrice || v?.product?.originalPrice || v?.price,
          // Lưu kích thước vật lý vào snapshot để syncToGHN có thể đọc khi variant đã bị xóa
          weight: v?.weight,
          length: v?.length,
          width: v?.width,
  ...
  private async validateDiscount(
    ...
    this.logger.debug(
      `[OrderCreation] Validating discount: ${discountCode} for userId: ${userId}`,
    );
    ...
    const hasUsed = await this.repository.hasUsedDiscount(
      userId || null,
      discount.id,
      guestEmail,
      guestPhone,
    ...
    this.logger.debug(
      `[OrderCreation] User/Guest has used discount ${discount.id}: ${hasUsed}`,
    );
    if (hasUsed) {
      throw new BadRequestException(
        'Bạn đã sử dụng mã giảm giá này cho đơn hàng trước đó',
      );
    }
